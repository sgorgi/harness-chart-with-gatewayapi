...
| config.CHAOS_DATA_BACKFILL_ENABLED | string | `"false"` | Enable/disable the parent_unique_id backfill cron job |
| config.CHAOS_DATA_BACKFILL_CRON_SCHEDULE | string | `"00:00"` | Daily schedule in HH:MM format (UTC) for backfill job |
| config.CHAOS_DATA_BACKFILL_BATCH_SIZE | string | `"50"` | Number of documents to process per batch |
| config.CHAOS_DATA_BACKFILL_BATCH_DELAY_SECONDS | string | `"2"` | Delay between batches to reduce DB load |
| config.CHAOS_DATA_BACKFILL_LOCK_DURATION_MINUTES | string | `"60"` | Redis distributed lock TTL in minutes |
| config.CHAOS_DATA_BACKFILL_MAX_CONCURRENT_COLLECTIONS | string | `"3"` | Max collections to process in parallel |
