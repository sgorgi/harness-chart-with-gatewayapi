...
{{- $tag := printf "%s" .Values.upgrader_docker_image.image.tag }}
{{- if .Values.global.fips }}
{{- $tag = printf "%s-fips" $tag }}
{{- end }}
{{- $image := dict "registry" .Values.upgrader_docker_image.image.registry "repository" .Values.upgrader_docker_image.image.repository "tag" $tag "digest" .Values.upgrader_docker_image.image.digest -}}
{{ include "common.images.image" (dict "imageRoot" $image "global" .Values.global) }}
{{- end }}
